...
void GLVertWidget::paintGLTransformationsVertexShaders() {
    // Clear the color and depth buffers.
        ...
        glm::vec3 position = glm::vec3(0.f, 0.5 + fabs( sin(3 * time) ), 0.f);
        glm::mat4 translation = glm::translate(position);
        GLint uniformLocation = glGetUniformLocation(m_program, "model");
        glUniformMatrix4fv(uniformLocation, 1, GL_FALSE, glm::value_ptr(translation));

        // TODO: Generate view matrix and pass it to vertex shader. (Task 4)
        glm::mat4 viewTranslation = glm::lookAt(eye, center, up);
        GLint viewLocation = glGetUniformLocation(m_program, "view");
        glUniformMatrix4fv(viewLocation, 1, GL_FALSE, glm::value_ptr(viewTranslation));

        // TODO: Generate projection matrix and pass it to vertex shader. (Task 4)
        glm::mat4 projectionTranslation = glm::perspective(fieldOfViewY, aspectRatio, nearClipPlane, farClipPlane);
        GLint projectionLocation = glGetUniformLocation(m_program, "perspective");
        glUniformMatrix4fv(projectionLocation, 1, GL_FALSE, glm::value_ptr(projectionTranslation));
        ...
        glm::vec3 positionSquare = glm::vec3(0.f, -0.3f, 0.f);
        glm::mat4 translationSquare = glm::translate(positionSquare);

        glm::mat4 scale = glm::scale(translationSquare, glm::vec3(2.0f, 2.0f, 2.0f));

        // TODO: Rotate the square to lie flat on the XZ plane. (Task 7)
        glm::mat4 rotate = glm::rotate(scale, 1.5708f, glm::vec3(1.0f, 0.0f, 0.0f));
        GLint uniformLocation3 = glGetUniformLocation(m_program, "model");
        glUniformMatrix4fv(uniformLocation3, 1, GL_FALSE, glm::value_ptr(rotate));
