...
void main(){
    ...
    float mult = lightIntensity * min(1 / (attConstant + attLinear * d + attQuadratic * pow(d,2.0f)), 1);

    float nL = dot(CameraSpace_normal_n, CameraSpace_toLight_n);
    vec3 ambComp = color * ambientIntensity;
    // to do: phong lighting model
//    fragColor = color * lightColor * nL * diffuseIntensity;

    vec3 diffuseComp = color * lightColor * nL * diffuseIntensity * mult;

    float dotResult = dot(CameraSpace_toEye_n, reflect(-CameraSpace_toLight_n,  CameraSpace_normal_n));
    vec3 specularComp = color * lightColor * specularIntensity * pow(max(dotResult, 0), shininess) * mult;

    fragColor = specularComp + ambComp + diffuseComp;
